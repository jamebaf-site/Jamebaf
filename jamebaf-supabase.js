...
function missingTagColumn(error){
  const m = (error && error.message || "").toLowerCase();
  return (m.includes("gender") || m.includes("model") || m.includes("size")) && (m.includes("column") || m.includes("schema"));
}
...
const JB = {
  ...
  async createProduct({ title, description, images, gender, model, size, price, pinned }) {
    ...
    if (error) {
      ...
      if (missingTagColumn(error)) throw new Error("ستون «gender»، «model» یا «size» در جدول products وجود ندارد — دستور SQL را در Supabase اجرا کنید.");
      throw error;
    }
  },
